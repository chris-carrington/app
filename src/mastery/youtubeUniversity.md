## What is Youtube University?
- Our curated list of helpful trade tutorials
- Each entry answers a specific trade question, provides a clear written answer and shares a YouTube video to see a demonstration of the written answer


---


<!--{"accordionStart":true}-->
## ⚡️ Electricity: How to turn electricity into rotation?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=hZxD1hwntqY" target="_blank">Jeremy Fielding Video</a> explores the induction motor, a device he describes as one of the most widely used inventions in the world, which earned Nikola Tesla a place in the National Inventors Hall of Fame. Fielding constructs a simple model using an aluminum disc and magnets to visually demonstrate how a rotating magnetic field drags the conductor with it, creating rotary motion
- This <a href="https://www.youtube.com/watch?v=CWulQ1ZSE3c" target="_blank">Jared Owen Video</a> provides a clear, 3D-animated explanation of how DC electric motors function by converting electrical energy into mechanical rotation
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## ⚡️ Electricity: How to make a solar power system?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=tXHKtBk9IE0" target="_blank">Wilson Forest Lands Video</a> is a beginner-friendly guide to building a DIY solar power system, emphasizing that it is simple enough for anyone to set up. The creator demonstrates the assembly of a basic system by focusing on three essential components (panels, battery, controller).
- This <a href="https://www.youtube.com/watch?v=dd7P1tdGyK4" target="_blank">Wilson Forest Lands Video</a> details how the creator successfully runs an off-grid solar power system year-round without the need for a generator, even during cloudy winter months
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## ⚡️ Electricity: How to connect splice wires together?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=ihpNCVk1lF0" target="_blank">Word of Advice Video</a> provides a practical guide on 10 different methods for splicing or reconnecting electrical wires.
<!--{"accordionEnd":true}-->


---


<!--{"accordionStart":true}-->
## 🪨 Concrete: How to pour a concrete pad?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=oxkLoHsf40I" target="_blank">Home RenoVision DIY Video</a> is a comprehensive DIY guide on pouring a concrete pad for an air conditioner unit that is intended to provide a long-lasting, stable base
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🪨 Concrete: How to repair concrete cracks?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=EKdJnN-Y_38" target="_blank">Michael Builds Video</a> is a practical, DIY guide for repairing concrete damage (cracks and voids). The video demonstrates how to achieve a professional, seamless finish.
<!--{"accordionEnd":true}-->


---


<!--{"accordionStart":true}-->
## 🌲 Carpentry: How to sand?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=qDYPGgjQmF4" target="_blank">Stumpy Nubs Video</a> is a comprehensive guide to woodworking sanding techniques
- This <a href="https://www.youtube.com/watch?v=Q0lEM_BOgmo" target="_blank">Stumpy Nubs Video</a> shows proper sanding techniques between coats of finish
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🌲 Carpentry: How to cut rabbets?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=7Qejzz9qI74" target="_blank">Stumpy Nubs Video</a> provides four essential tips for achieving clean and accurate router rabbets in our woodworking projects
<!--{"accordionEnd":true}-->



<!--{"accordionStart":true}-->
## 🌲 Carpentry: How to create a box?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=QyokX-BK7FQ" target="_blank">Stumpy Nubs Video</a> shows how to build a strong, self-aligning box, drawer, or cabinet using a locking rabbet joint and a hidden groove joint. This method is designed to simplify project assembly and ensure durability without the need for complex, visible stop cuts.
- This <a href="https://www.youtube.com/watch?v=HQXHzvJNsoA" target="_blank">Stumpy Nubs Video</a> shows an efficient, fast, and simple method for constructing locking rabbet drawer boxes using a single table saw setup, which he refers to as the "half-half-half" drawer method.
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🌲 Carpentry: How to join with pocket holes?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=OmMN_XKJ4nc" target="_blank">Workshop Companion Video</a> provides a comprehensive guide to using pocket hole joinery. The host, Nick Engler, breaks down the technique into several key areas (strength testing, terminology & design, screwology, jigs and alternative methods).
- This <a href="https://www.youtube.com/watch?v=bOrQ9lH8qSc" target="_blank">WoodWorkWeb Video</a> provides a practical guide on common mistakes to avoid when using pocket hole joinery. The goal is to help beginners achieve more consistent and reliable results in their woodworking projects.
- This <a href="https://www.youtube.com/watch?v=oqjvCCHxNOo" target="_blank">Anika's DIY Life Video</a> demonstrates four effective methods for filling pocket holes in woodworking projects to achieve a clean, professional finish. Anika emphasizes that while the best practice is to place pocket holes where they won't be seen, these techniques are essential for situations where the joints remain exposed.
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🌲 Carpentry: Is it safe to build w/ pallet wood?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=4IblRBJMh-k" target="_blank">Dainer Made Video</a> addresses common concerns and misconceptions regarding the use of shipping pallets for DIY projects and ultimately concludes that pallet wood is a viable resource that just requires a bit of extra effort, cleanup, and caution
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🌲 Carpentry: How to find the center of a board?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=s6QTmcAWLZs" target="_blank">Tony's Woodshop Video</a> demonstrates a quick and accurate technique for finding the center of a board using a speed square
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🌲 Carpentry: What woodworking glue is nontoxic?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=75Mm-DE_EoA" target="_blank">Workshop Companion Video</a> shows the safety of common wood glues when used for kitchenware like cutting boards. While few manufacturers obtain official FDA Food Safe ratings, the video categorizes adhesives by their chemical makeup and safety for food contact.
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🌲 Carpentry: How to cut any angle with a skill saw?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=YQv8pKuc1W8" target="_blank">Wood Bully Video</a> demonstrates a technique for using a circular saw to make precise angled cuts. The method involves using a speed square paired with stair gauges (also referred to as stair nuts) to lock in any desired angle.
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🌲 Carpentry: How to change the blade on a skill saw?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=d2V2OptTYtE" target="_blank">365 Red Leaf Video</a> provides a quick tutorial on how to change a circular saw blade safely and efficiently
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🌲 Carpentry: When to not use pressure treated lumber?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=qZdrFLq3-ws" target="_blank">Wilson Forest Lands Video</a> explains why pressure-treated lumber may not be the ideal choice for projects requiring significant structural strength, specifically when supporting heavy items like shipping containers
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🌲 Carpentry: Why should we not use furnisure polish on wooden surfaces?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=yog33yzzMN0" target="_blank">Workshop Companion Video</a> explains why repair experts advise against using commercial furniture polish on wooden surfaces, why paste wax is a better alternative and how to use it optimally. 
<!--{"accordionEnd":true}-->


---




<!--{"accordionStart":true}-->
## 🚪 Doors: How to install a door?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=za1xCbZIr2A" target="_blank">BAS Renovations Video</a> provides a comprehensive guide on how to achieve a strong and professional door installation. The core takeaway is that a strong installation depends on properly tying the door into the structural framing of the wall, rather than just the type of door used.
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🪟 Windows: How to install a window?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=Q4HOB4E-fJc" target="_blank">BAS Renovations Video</a> provides a step-by-step guide on how to properly install a window, focusing on flashing techniques to ensure a water-tight installation.
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## 🪚 Flooring: How to install vinyl plank floors?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=Q4HOB4E-fJc" target="_blank">BAS Renovations Video</a> provides a step-by-step guide on how to properly install a window, focusing on flashing techniques to ensure a water-tight installation.
- This <a href="https://www.youtube.com/watch?v=Q4HOB4E-fJc" target="_blank">Fix This Build That Video</a> is a beginner-friendly guide to installing vinyl plank flooring in a laundry room. The creator provides a step-by-step walkthrough, emphasizing planning, preparation, and the use of basic tools to achieve professional-looking results.
<!--{"accordionEnd":true}-->


<!--{"accordionStart":true}-->
## ⛩️ Fencing: How to install a vinyl or wood fence?
<!--{"accordionBody":true}-->
- This <a href="https://www.youtube.com/watch?v=qSNGtAtRpTc" target="_blank">BAS Renovations Video</a> provides a comprehensive guide on installing a durable vinyl or wood fence.
<!--{"accordionEnd":true}-->
