...
import { db, Trade, JobStatus, LeadStatus, StaffPosition, StaffEndReason, ObjectiveColumn, ObjectiveTag, ObjectiveActivityType } from '@src/db'
...
const objectiveTags = [
  ...
  { value: 'Ready for QA', order: 2, bgHex: '#E1DBFE', fgHex: '#5027FF' },
  { value: 'In QA', order: 3, bgHex: '#FEF3C7', fgHex: '#92400E' },
  { value: 'Failed QA', order: 4, bgHex: '#FEE2E2', fgHex: '#991B1B' },
  { value: 'Passed QA', order: 5, bgHex: '#F3F9F5', fgHex: '#018A5D' },
  { value: 'Completed', order: 6, bgHex: '#DCFCE7', fgHex: '#166534' },
  { value: 'Archived', order: 7, bgHex: '#F1F5F9', fgHex: '#475569' }
]

const objectiveActivityTypes = [
  { value: 'Assignee Added' },
  { value: 'Assignee Removed' },
  { value: 'Column Changed' },
  { value: 'Tag Added' },
  { value: 'Tag Removed' },
  { value: 'Comment Added' },
  { value: 'Objective Created' },
]
...
await Promise.all([
  ...
  db.insert(ObjectiveActivityType).values(objectiveActivityTypes),
])
