// app/src/db/putObjective.ts

import { eq, and, inArray } from 'drizzle-orm'
import { Objective, Objective__Tag, Objective__Assignee, type Transaction } from '@src/db'


/** Upsert an Objective – delegates to `insert` or `update` based on presence of `id` */
export async function putObjective(tx: Transaction, input: PutObjectiveInput,): Promise<number> {
  return (input.id)
    ? updateObjective(tx, input as PutObjectiveInput & { id: number })
    : insertObjective(tx, input)
}


/** Insert a new Objective */
export async function insertObjective(tx: Transaction, input: Omit<PutObjectiveInput, 'id'>): Promise<number> {
...
async function insertObjectiveChildren(tx: Transaction, input: Omit<PutObjectiveInput, 'id'>, objectiveId: number) {
...
export async function updateObjective(tx: Transaction, input: PutObjectiveInput & { id: number }): Promise<number> {
  ...
  await Promise.all([ // update children
    updateAssignees(tx, id, assigneeIds),
    updateTags(tx, id, tagIds),
  ])
...
export type PutObjectiveInput = {
  /** If provided, update the existing objective; otherwise insert a new one. */
  id?: number
  columnId: number
  /** Person who created the objective (used only on insert). */
  createdBy: number
  title: string
  description?: string
  order: number
  /** List of Person IDs to assign – empty array clears all. */
  assigneeIds: number[]
  /** List of ObjectiveTag IDs – empty array clears all. */
  tagIds: number[]
}
